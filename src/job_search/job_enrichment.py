...
def _sqlite_row_for_raw_job(
    job: dict[str, Any], cv_id: str = "default", scrape_id: str = "legacy"
...
def _sqlite_row_for_enriched_job(
    job: Job, cv_id: str = "default", scrape_id: str = "legacy"
) -> tuple[str, dict[str, Any], str]:
...
def _ensure_sqlite_columns(
    ...
    table_definition += ", PRIMARY KEY (" + ", ".join(key_columns) + ")"
    ...
    if existing_key_columns == list(key_columns):
        return
    ...
    if copied_columns:
        select_columns = [
            f"COALESCE({column}, 'legacy')" if column in key_columns else column
            for column in copied_columns
        ]
        columns = ", ".join(copied_columns)
        conn.execute(
            f"INSERT INTO {table_name} ({columns}) SELECT {', '.join(select_columns)} FROM {legacy_table}"
        )
    ...
    *,
    ...
    try:
        conn.execute(
            ...
            "scrape_id TEXT PRIMARY KEY, cv_id TEXT NOT NULL, started_at TEXT NOT NULL, "
            "completed_at TEXT NOT NULL)"
        )
        now = datetime.now(timezone.utc).isoformat()
        conn.execute(
            "INSERT OR IGNORE INTO scrapes(scrape_id, cv_id, started_at, completed_at) "
            "VALUES (?, ?, ?, ?)",
            (scrape_id, cv_id, now, now),
        )
        raw_fields = [
            "cv_id TEXT",
            "scrape_id TEXT",
        ...
        _ensure_sqlite_columns(conn, "raw_jobs", raw_fields, ("cv_id", "scrape_id", "job_key"))
        ...
        _ensure_sqlite_columns(conn, "enriched_jobs", enriched_fields, ("cv_id", "scrape_id", "job_key"))

        for raw_job in raw_jobs:
            _, row, _ = _sqlite_row_for_raw_job(raw_job, cv_id, scrape_id)
            columns = ", ".join(row.keys())
            placeholders = ", ".join("?" for _ in row)
            values = tuple(row.values())
            conn.execute(
                f"INSERT INTO raw_jobs ({columns}) VALUES ({placeholders}) ON CONFLICT(cv_id, scrape_id, job_key) DO UPDATE SET "
                + ", ".join(f"{col} = excluded.{col}" for col in row if col not in {"cv_id", "scrape_id", "job_key"}),
                values,
            )

        for enriched_job in enriched_jobs:
            if isinstance(enriched_job, Job):
                _, row, _ = _sqlite_row_for_enriched_job(enriched_job, cv_id, scrape_id)
            else:
                ...
                _, row, _ = _sqlite_row_for_enriched_job(job, cv_id, scrape_id)
            columns = ", ".join(row.keys())
            placeholders = ", ".join("?" for _ in row)
            values = tuple(row.values())
            conn.execute(
                f"INSERT INTO enriched_jobs ({columns}) VALUES ({placeholders}) ON CONFLICT(cv_id, scrape_id, job_key) DO UPDATE SET "
                + ", ".join(f"{col} = excluded.{col}" for col in row if col not in {"cv_id", "scrape_id", "job_key"}),
                values,
            )
